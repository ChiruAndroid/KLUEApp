            ...
            public void onClick(View v) {
                if (!preferenceUtils.getbooleanFromPreference(PreferenceUtils.LOG_OUT, true)) {
                    if (!list.get(position).isChecked()) {
                        Anniversary.product_arrayList.get(position).setChecked(true);
                        Anniversary.product_arrayList.get(position).setIs_wish_list(1);
                        Whishlist.addToWishList(context, position, fragmentName);
                        holder.heart.setImageResource(R.mipmap.whishlist_hvr);
                        notifyItemChanged(position);
                        notifyDataSetChanged();
                    } else {
                        Anniversary.product_arrayList.get(position).setChecked(false);
                        Anniversary.product_arrayList.get(position).setIs_wish_list(0);
